...
public class Client {
    ...
    public Client(String ip, int port) throws IOException, InterruptedException {
        ...
        Thread receiver = new Thread(new ReceiveMessageFromServer(out, in, socket));
        Thread sender = new Thread(new SendMessageToServer(out, in, socket));
        receiver.start();
        sender.start();
        receiver.join();
        sender.join();
    ...
    private class ReceiveMessageFromServer extends Thread {
        ...
        private void sendMessage() throws IOException {
            while (true) {
                ...
                JSONObject messageObject = JMessageWorker.makeJSONObject(message, RESET);
                message = messageObject.toJSONString();
                out.println(message);
    ...
    public class SendMessageToServer extends Thread {
        ...
        private void receiveMessage() throws IOException, InterruptedException {
            while (in.hasNextLine()) {
                ...
                JMessage jMessage = JMessageWorker.parseToObject(message);
                message = jMessage.getMessage();
                String color = jMessage.getColor();
